...
-   **`New Code/`**: 
    -   **`Simulator App/`**: An interactive Streamlit app tailored for easy viewing and parameterized execution. 
    -   **`New Implementation/`**: A standalone Python script (`simulated_paths.py`).
-   **`Data/`**: Contains the necessary data files for validation.
-   **`Original Code/`**: Reference implementations (Python Jupyter Notebook sent originally).
