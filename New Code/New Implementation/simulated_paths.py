...
if N_PATHS_PREDICTABLE > 0:
    ...
    returns_pred, signal_pred, returns_pred_annual = simulate_predictable_paths(N_SIMULATIONS, T, H)

if N_PATHS_IID > 0:
    print("Generating i.i.d. paths...")
    returns_iid, signal_iid, returns_iid_annual = simulate_iid_paths(N_SIMULATIONS, T, H)
...
returns_pred_selected = signal_pred_selected = None
returns_iid_selected = signal_iid_selected = None
    ...
    for i in range(len(idx_pred)):
        for t in range(T):
            all_data.append({
                ...
                'signal': signal_pred_selected[t, i],
    ...
    for i in range(len(idx_iid)):
        for t in range(T):
            all_data.append({
                ...
                'signal': signal_iid_selected[t, i],
                'is_observation': 1 if t % H == 0 else 0
            })
...
if SAVE_EXAMPLE_PLOTS and (N_PATHS_PREDICTABLE > 0 or N_PATHS_IID > 0):
    ...
    if n_pred_to_plot > 0:
        for i in range(n_pred_to_plot):
            ...
            s_plot = signal_pred_selected[0:T_is:H, i] * 100
    ...
    if n_iid_to_plot > 0:
        for i in range(n_iid_to_plot):
            ...
            s_plot = signal_iid_selected[0:T_is:H, i] * 100
